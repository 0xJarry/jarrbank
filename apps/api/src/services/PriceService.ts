...
interface PriceProvider {
  name: 'moralis' | 'defiLlama' | 'coinmarketcap';
  fetchPrice: (tokenAddress: string, chainId: ChainId) => Promise<number | null>;
  fetchBatchPrices: (tokens: { address: string; chainId: ChainId }[]) => Promise<Map<string, number>>;
}
...
export class PriceService extends EventEmitter {
  ...
  private priceCacheTTL = 15; // 15 seconds
  ...
  private createMoralisProvider(): PriceProvider {
    return {
      name: 'moralis',
      fetchPrice: async (tokenAddress: string, chainId: ChainId) => {
      ...
      fetchBatchPrices: async (tokens) => {
        ...
        const chainGroups = new Map<ChainId, { address: string; chainId: ChainId }[]>();
        ...
        await Promise.all(
          Array.from(chainGroups.entries()).map(async ([chainId, chainTokens]) => {
            const chainMap: Record<ChainId, string> = {
              [CHAIN_IDS.ETHEREUM]: '0x1',
              [CHAIN_IDS.ARBITRUM]: '0xa4b1',
              [CHAIN_IDS.AVALANCHE]: '0xa86a'
  ...
  private createDefiLlamaProvider(): PriceProvider {
    return {
      name: 'defiLlama',
      fetchPrice: async (tokenAddress: string, chainId: ChainId) => {
        try {
  ...
  private createCoinMarketCapProvider(): PriceProvider {
    return {
      name: 'coinmarketcap',
      fetchPrice: async (tokenAddress: string, chainId: ChainId) => {
        ...
        try {
          const platformMap: Record<ChainId, number> = {
      ...
      fetchBatchPrices: async (tokens) => {
        ...
        if (uniqueAddresses.size === 0) return prices;
        ...
        try {
          ...
          if (response.ok) {
            ...
            Object.values(data.data || {}).forEach((tokenData: any) => {
  ...
  async getBatchTokenPrices(
    ...
    const pricesMap = new Map<string, TokenPriceData>();
    ...
    for (const provider of this.providers) {
      const remainingTokens = uncachedTokens.filter(
        token => !pricesMap.has(`${token.chainId}:${token.address.toLowerCase()}`)
      );
      ...
      await Promise.all(
        Array.from(batchPrices.entries()).map(async ([key, price]) => {
    ...
    const missingTokens = uncachedTokens.filter(
      token => !pricesMap.has(`${token.chainId}:${token.address.toLowerCase()}`)
    );
  ...
  async getNativeTokenPrices(): Promise<Map<ChainId, number>> {
    const prices = new Map<ChainId, number>();
    
    const nativeTokens = [
      { symbol: 'ETH', coinId: 'ethereum', chainIds: [CHAIN_IDS.ETHEREUM, CHAIN_IDS.ARBITRUM] },
      { symbol: 'AVAX', coinId: 'avalanche-2', chainIds: [CHAIN_IDS.AVALANCHE] }
    ];

    await Promise.all(
      nativeTokens.map(async ({ symbol, coinId, chainIds }) => {
