...
import {
  ...
  AssetCategory,
  SyncResult
} from '../../../../packages/shared/src/types/portfolio';
import {
  calculateTokenValue,
  parsePriceToUSDCents,
  aggregateTokenValues,
  calculatePortfolioBreakdown
} from '../../../../packages/shared/src/utils/format';
...
export class PortfolioAggregator extends EventEmitter {
  ...
  async aggregatePortfolio(
    ...
    try {
      const balanceData = await this.tokenBalanceService.detectTokenBalances(
        walletAddress,
        chainIds
      );
      ...
      for (const chainData of balanceData) {
        ...
        if (chainData.nativeBalance > 0n) {
          ...
          const priceInCents = nativePrice ? parsePriceToUSDCents(nativePrice) : 0n;
          const valueUSD = nativePrice ? calculateTokenValue(
            chainData.nativeBalance,
          ...
          tokens.push({
            ...
            valueUSD,
            category: AssetCategory.BLUECHIP,
            lastUpdatedAt: new Date()
          });
        }

        for (const tokenData of chainData.tokens) {
          if (!tokenData.hasBalance || !tokenData.balance) continue;
          ...
          if (metadata && priceData) {
            const priceInCents = parsePriceToUSDCents(priceData.priceUSD);
            const valueUSD = calculateTokenValue(
              tokenData.balance,
              metadata.decimals,
              priceInCents
            );
        ...
        const totalValue = aggregateTokenValues(tokens);
  ...
  private calculateAssetComposition(
    ...
    const categoryMap = new Map<AssetCategory, { value: bigint; count: number }>();
  ...
  private calculateHealthScore(composition: AssetComposition[]): number {
    let score = 100;

    const stablecoinPercentage = composition
      .find(c => c.category === AssetCategory.STABLECOIN)?.percentage || 0;
    const memePercentage = composition
      .find(c => c.category === AssetCategory.MEME)?.percentage || 0;
  ...
  async getPortfolioSummary(
    ...
    const chainBreakdown = calculatePortfolioBreakdown(
      portfolios.map(p => ({
        chainId: p.chainId,
    ...
    const aggregatedComposition = new Map<AssetCategory, AssetComposition>();
    ...
    return {
      totalValueUSD,
      chainBreakdown: chainBreakdown as { chainId: ChainId; valueUSD: bigint; percentage: number }[],
      topTokens,
      assetComposition
    };
  }
  ...
  async syncPortfolioData(
    ...
    try {
      ...
      const portfolios = await this.aggregatePortfolio(
        userId,
        walletAddress,
        [chainId]
      );
