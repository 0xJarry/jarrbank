...
import { TokenMetadata, TokenBalance, AssetCategory } from '../../../../packages/shared/src/types/portfolio';
...
export class TokenBalanceService extends EventEmitter {
  ...
  private commonTokens: Map<ChainId, string[]>;
  ...
  private initializeCommonTokens(): Map<ChainId, string[]> {
    const tokens = new Map<ChainId, string[]>();
    
    tokens.set(CHAIN_IDS.ETHEREUM, [
    ...
    tokens.set(CHAIN_IDS.AVALANCHE, [
  ...
  async detectTokenBalances(
    walletAddress: string,
    chainIds: ChainId[]
  ): Promise<ChainTokenBalances[]> {
    const results = await Promise.all(
      chainIds.map(chainId => this.detectChainTokens(walletAddress, chainId))
    );
    
    return results;
  }

  private async detectChainTokens(
    walletAddress: string,
    chainId: ChainId
  ): Promise<ChainTokenBalances> {
    ...
    const tokens: TokenDetectionResult[] = [];

    if (alchemyTokens.length > 0) {
      tokens.push(...alchemyTokens);
    } else {
      const commonTokenAddresses = this.commonTokens.get(chainId) || [];
      const tokenBalancePromises = commonTokenAddresses.map(tokenAddress =>
        this.getTokenBalance(walletAddress, tokenAddress, chainId)
      );
    ...
    const result: ChainTokenBalances = {
      chainId,
      walletAddress,
      nativeBalance,
      tokens
    ...
    // @ts-ignore
    if (typeof (this.redisClient as any).setEx === 'function') {
      // @ts-ignore
      await (this.redisClient as any).setEx(cacheKey, 15, JSON.stringify(cacheData));
    } else {
      await (this.redisClient as any).setex(cacheKey, 15, JSON.stringify(cacheData));
    }
  ...
  private async getNativeBalance(walletAddress: string, chainId: ChainId): Promise<bigint> {
    try {
  ...
  private async fetchAlchemyTokenBalances(
    walletAddress: string,
    chainId: ChainId
    ...
    try {
      ...
      return (data as any).result.tokenBalances
        .filter((tb: any) => tb.tokenBalance && tb.tokenBalance !== '0x0')
        .map((tb: any) => ({
  ...
  async batchFetchTokenBalances(
    ...
    await Promise.all(
      batches.map(async (batch) => {
        const balanceOfSelector = '0x70a08231';
        const requests: RpcRequest[] = batch.map((tokenAddress, index) => ({
          id: index,
          method: 'eth_call',
          params: [{
            to: tokenAddress,
  ...
  async getFormattedBalances(
    ...
    const totalTokensFound = chainBalances.reduce(
      (sum, chain) => sum + chain.tokens.filter(t => t.hasBalance).length,
      0
    );
